...
| 检测项 | 检测方法 | 风险等级 |
| -- | -- | -- | 
| 通信库算子性能检测 | 使用 nccl-test 测试 NCCL 通信性能，输出带宽指标，验证多 GPU 协同效率。  |  高（影响模型训练速度） |
| Roce/IB 通信带宽和时延检测 | 用 ib_write_bw/ib_read_bw 测带宽，ib_write_lat/ib_read_lat 测时延，验证网络传输效率。  |  高（影响分布式训练） |
| CPU性能检测 | stress-ng 压测满载性能  |  中（影响计算效率） |
| 内存性能检测 | mbw 测带宽  | 中（触发 OOM 风险） |
| 磁盘io性能检测 | fio 模拟随机 / 顺序读写压测  | 中（影响数据读写速度） |

故障检测：

| 检测项 |  检测方法 | 解决方法  | 风险等级 |
| -- | -- | -- |  -- | 
| GPU ECC Errors | nvidia-smi -q -d ECC 查 “Correctable Errors”，识别可纠正内存错误。 | **自愈**：执行 nvidia-smi --reset-gpu 重启驱动，或 nvidia-smi -pm 1 启用持久模式降低错误；**人工兜底**：长期高错误时，停机检查 GPU 内存模块，必要时更换 GPU。 |  低（可自动纠错） |
| GPU Uncorrectable Errors | nvidia-smi -q -d ECC 查 “Uncorrectable Errors”，识别不可恢复内存故障。 |  **自愈**：立即执行 nvidia-smi -i [GPU_ID] -c EXCLUSIVE_PROCESS 隔离故障 GPU，自动触发节点重启；**人工兜底**：更换故障 GPU 卡，重新纳管节点并验证 ECC 状态。 | 高（直接中断训练） |
| GPU 掉卡检测 | nvidia-smi -L 列出现有 GPU，对比基线数量，识别掉卡。  | **自愈**：自动禁止调度 → 排干节点 Pod → 重启节点（触发热插拔扫描）；**人工兜底**：检查 GPU 物理插槽、供电，重新插拔卡，更新驱动 / 固件后验证。 |  高（训练任务失败） |
| NVLink P2P状态检测 | nvidia-smi nvlink -s 查 NVLink 连接状态、带宽，识别链路异常。 | **自愈**：自动禁止调度 → 重启节点；**人工兜底**：检查 NVLink 桥接器物理连接，更新 GPU 驱动 / 固件，必要时更换硬件。 |  高（多 GPU 通信中断）	|
| nvidia-peermem 模块加载检测  | lsmod grep nvidia_peermem 检查模块是否加载，缺失则影响 P2P 内存访问。| **自愈**：自动执行 modprobe nvidia_peermem 加载模块，写入 /etc/modules-load.d/ 确保开机自启；**人工兜底**：更新 GPU 驱动，验证模块加载状态。 |  中（影响内存性能） |
| FabricManager 服务检测  |systemctl status nvidia-fabricmanager 查服务状态，异常则影响 NVLink 通信。 | **自愈**：自动执行 systemctl restart nvidia-fabricmanager 重启服务；**人工兜底**：检查日志 /var/log/nvidia-fabricmanager/，升级 DCGM 或更换故障组件。|  中（多 GPU 协同异常） |
| 网卡掉卡检测 | ibdev2netdev 映射 IB/RoCE 设备，对比基线识别掉卡。 | **自愈**：自动禁止调度 → 排干节点 → 重启节点；**人工兜底**：检查网卡供电、物理接口，更新驱动 / 固件，必要时更换网卡。 |   高（网络通信中断） |
| 网卡状态检测  | ip link show 查网卡状态（UP/DOWN），ethtool 查速率、双工模式。 | **自愈**：自动执行 ip link set [网卡名] down && up 软重启网卡；**人工兜底**：硬重启（更换网卡），检查交换机端口、线缆，验证网络连通性。 | 高（网络中断） |
...
| 节点重启 | 上报Kubernetes事件，并为节点添加NoExecute污点。|
| 重新检测 | 上报Kubernetes事件。|
...
不同检测项可能需要不通测参数,在页面设计时那种设计方式比较合理一些?

1. 所有检测项都在一个页面,根据需要检测的参数项来填写参数.
2. 不同检测项在不同的页面,每个页面只负责一个检测项.(无参数项的可以合并成一个)


检测结果可以根据表格的形式进行展示,统一展示风格.
...
实现方案: 基于在当前的环境检测逻辑上,添加新的检测项.

当前任务级别的环境检测流程如下:
1. 将任务pod申请的资源通过init容器挂载进去.
2. 在init容器中执行通信库算子通信检测和矩阵计算的脚本.
3. 更新检测结果到pod 注解中,最后同步到CR status中.
...
查看 GPU 温度与状态：
使用 nvidia-smi 命令查看 GPU 温度（nvidia-smi --query-gpu=temperature.gpu --format=csv,noheader）。
若温度超过阈值（通常为 85°C 以上），可能触发硬件降速。
确认降速状态：
通过 nvidia-smi --query-gpu=clocks.current.sm --format=csv,noheader 查看当前 SM 频率，对比默认频率（可通过 nvidia-smi --query-gpu=clocks.default.sm --format=csv,noheader 获取）。若当前频率显著低于默认值，可能已降速。

![alt text](image.png)

### nvidia-fabricmanager 服务组件检测

```bash
systemctl status nvidia-fabricmanager
● nvidia-fabricmanager.service - NVIDIA fabric manager service
     Loaded: loaded (/lib/systemd/system/nvidia-fabricmanager.service; enabled; vendor preset: enabled)
     Active: active (running) since Tue 2025-01-21 09:54:22 CST; 4 months 17 days ago
   Main PID: 3421 (nv-fabricmanage)
      Tasks: 19 (limit: 629145)
     Memory: 21.7M
        CPU: 2h 10min 49.580s
     CGroup: /system.slice/nvidia-fabricmanager.service
             └─3421 /usr/bin/nv-fabricmanager -c /usr/share/nvidia/nvswitch/fabricmanager.cfg
