...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            torch.LongTensor([is_next])  # 注意：这里需要用列表包裹，否则维度不对
        )
...
def train(rank, world_size, dataset, vocab_size):
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            # 直接获取Tensor并转移到GPU（修复后这里不会报错）
            input_ids = batch[0].to(rank)
            segment_ids = batch[1].to(rank)
            masked_tokens = batch[2].to(rank)
            masked_pos = batch[3].to(rank)
            is_next = batch[4].squeeze().to(rank)  # 注意：这里加squeeze()去除多余维度
