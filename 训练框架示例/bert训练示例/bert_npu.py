...
def setup(rank, world_size):
    ...
    init_process_group("hccl", rank=rank, world_size=world_size)  # NPU使用hccl后端
    device = f"npu:{rank}"  # 统一设备标识
    torch.npu.set_device(device)
    return device  # 返回设备标识供后续使用
...
# 训练函数（使用统一设备变量）
# --------------------------
def train(rank, world_size, dataset, vocab_size):
    device = setup(rank, world_size)  # 获取统一设备标识（如"npu:0"）
...
def main():
    ...
    if not torch.npu.is_available():
        raise RuntimeError("未检测到NPU设备，请确保已安装Ascend PyTorch环境和torch_npu")
    ...
    print(f"使用 {world_size} 张NPU进行训练")

    mp.spawn(
        train,
        args=(world_size, dataset, vocab_size),
        nprocs=world_size,
        join=True
    )
...
if __name__ == "__main__":
    ...
    torch.npu.manual_seed_all(42)  # NPU随机种子初始化
