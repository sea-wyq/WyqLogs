...
（1） 提供多云监控监控方案，该方案需要具备如下功能：
   1. 更好的兼容 Prometheus、Grafana 等开源工具，减少迁移成本，学习成本。
   2. 解决成员集群下线后，部分数据（两个小时，prometheus机制导致）丢失的情况。
   3. 方案能提供比Prometheus更加高效的数据存储和压缩能力。在相同的存储空间下保存更长时间的数据。
   4. 在高频写入的场景下，技术方案能够水平扩展，以支持更高的写入吞吐量，同时确保数据的一致性和可靠性。
（2）自定义网络exporter：当前环境采集网络监控数据的组件时node-exporter,如果单纯的只提高采集网络监控数据的exporter的采集频率，可以通过自定义网络监控的方式，将网络监控的功能抽取出来，减少资源使用。
（3） Prometheus升级验证。
...
Prometheus 在处理并发查询请求时，性能较差，且没有多组件部署的能力。因此，需要调研新的多云监控方案，以解决现有问题。
...
通过比较社区活跃度，部署难度，性能分析，和运维难度等方面来进行对比，`VictoriaMetrics`方案最优，对比结果如下表：
...
1. 子集群Prometheus CR配置修改
    1. 添加 remotewrite配置参数，指向主控的监控组件。
    ```bash
    remoteWrite:
        - url: http://thanos-receive-ingestor-default.thanos.svc.cluster.local:19291/api/v1/receive # thanos
        - url: http://vmsingle-victoria-metrics-single-server.default.svc.cluster.local:8428/api/v1/write  # vm-signal
        - url: http://vm-cluster-victoria-metrics-cluster-vminsert.vm.svc.cluster.local:8480/insert/0/prometheus # vm-cluster
        - url: http://mimir-nginx.mimir.svc:80/api/v1/push # mimir
    ```
    ...
    external_labels:
        clusterIP: localhost
    ```
    > 注：需要考虑多个集群不在同一局域网下，如何使用公网ip来配置remotewrite ？何种方式更新上面配置？
2. Resource-Manager 修改项
...
当前监控部署的组件如下所示：
- kube_state_metrics：  收集k8s CR的状态信息。
- dcgm-exporter： 收集GPU相关信息。
- node_exporter： 收集节点的网络，存储，磁盘，CPU，内存等信息。
- cadvisor(kubelet)：收集容器的网络，存储，磁盘，CPU，内存等信息。
- prometheus：监控数据存储，查询，告警。
- alertmanager：告警管理。
- grafana：数据可视化。
- resource-manger：监控指标查询和数据后处理。
...
![sig-2-2025-03-21](https://fourt-wyq.oss-cn-shanghai.aliyuncs.com/images/sig-2-2025-03-21.png)

优化思路：
- user-prometheus只存储hero-user命名空间下的监控数据（持久化时间长），system-prometheus存储所有命名空间下的监控数据（持久化时间短）。提高存储空间的使用，降低存储压力。

###  6.2. 多云集群监控部署（主控对接多个子集群）
当前多云集群监控部署方案架构如下：
...
(1) 子集群不提供对外服务，只提供资源。
![mutli-2-2025-03-21](https://fourt-wyq.oss-cn-shanghai.aliyuncs.com/images/mutli-2-2025-03-21.png)

可直接部署单个Prometheus组件，只写入hero-userm命名空间下的监控数据。

(2) 子集群和主控集群都提供服务。
![mutli-3-2025-03-21](https://fourt-wyq.oss-cn-shanghai.aliyuncs.com/images/mutli-3-2025-03-21.png)

子集群部署user-prometheus和system-prometheus，user-prometheus只上传hero-user命名空间下的用户数据。

> 单集群监控组件仍然选择prometheus的原因: 具有可视化的页面，方便查看监控数据的状态以及问题定位


###  6.3. 多云集群监控部署（子集群对接多个主控）
...
不同主控集群的任务下发到不同的用户命名空间，监控组件根据命名空间将数据分流到不同主控。
例如：
...
```bash
    remoteWrite:
    - url: http://remote-write-1.example.com/api/v1/write
      writeRelabelConfigs:
        - sourceLabels: [__meta_kubernetes_namespace]
          regex: namespace-1
          action: keep
    - url: http://remote-write-2.example.com/api/v1/write
      writeRelabelConfigs:
        - sourceLabels: [__meta_kubernetes_namespace]
          regex: namespace-2
          action: keep
```
结构图如下：
![mutli-4-2025-03-21](https://fourt-wyq.oss-cn-shanghai.aliyuncs.com/images/mutli-4-2025-03-21.png)
...
### Prometheus agent / grafan agent /vm agent


结论先行，文档需要考虑不同人群。


###  3.1. Loki + Promtail 使用现状

最新版本promtail已经弃用，详细参考[Loki Promtail](https://grafana.com/docs/loki/latest/send-data/promtail/)

可替代Promtail的日志抓取组件如下：
- Fluent Bit
- Fluentd
- Logstash
- Grafana Alloy
