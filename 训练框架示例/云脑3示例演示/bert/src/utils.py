    ...
    def step_end(self, run_context):
        ...
        if self._dataset_size > 0:
            ...
            if percent == 0:
                percent = 1
                epoch_num -= 1
            print("time: {}, epoch: {}, current epoch percent: {}, step: {}, outputs are {}"
                  .format(datetime.datetime.now(), int(epoch_num), "%.3f" % percent, cb_params.cur_step_num,
...
class EvalCallBack(Callback):
    ...
    def epoch_end(self, run_context):
        ...
        total_samples = cb_params.cur_step_num * self.global_batch
        res = self.model.eval(self.eval_ds, dataset_sink_mode=True)
        res = res['bert_acc']
        print("====================================", flush=True)
        print(f"Time: {datetime.datetime.now()} Accuracy is: ", "%.6f" % res, ", current samples is: ",
              total_samples)
        print("====================================", flush=True)
