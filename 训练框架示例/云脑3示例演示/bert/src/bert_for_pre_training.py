...
class BertPretrainingLoss(nn.Cell):
    ...
    def construct(self, prediction_scores, seq_relationship_score, masked_lm_ids,
        ...
        # total_loss
        total_loss = masked_lm_loss + next_sentence_loss #* tmp
...
class BertTrainOneStepWithLossScaleCell(nn.TrainOneStepWithLossScaleCell):
    ...
    def construct(self,
        ...
        if sens is None:
            ...
            scaling_sens = sens
        flag = ops.cast(ops.sum(masked_lm_weights), ms.bool_)
        flag_float = ops.cast(flag, ms.float32)
        if not flag:
            return flag_float.unsqueeze(-1), flag, scaling_sens.value()
        loss = self.network(input_ids,
                            input_mask,
                            token_type_id,
                            next_sentence_labels,
...
class BertPretrainEval(nn.Cell):
    ...
    def construct(self,
        ...
        _, index = mlm.argmax_with_value(axis=-1)
