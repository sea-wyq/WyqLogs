...
from src.dataset import create_bert_dataset, create_eval_dataset, CreateEvalDatasetInput
...
os.environ["GLOG_v"] = "1"
print(os.getenv("GLOG_v"))
print(os.getenv("P128"))


class AllreduceSync(nn.Cell):
    def __init__(self):
        super(AllreduceSync, self).__init__()
...
def run_pretrain():
    ...
    new_repeat_count = cfg.epoch_size * ds.get_dataset_size() // cfg.data_sink_steps # 100w  -> 100w/100
    ...
    if cfg.train_with_eval == 'true':
        ...
        inputs = CreateEvalDatasetInput(batchsize=cfg.batch_size, device_num=device_num, rank=rank,
                                        data_dir=cfg.eval_data_dir, schema_dir=cfg.schema_dir,
                                        dataset_format=cfg.dataset_format, num_samples=cfg.num_samples)
        eval_ds = create_eval_dataset(inputs)
    ...
    new_repeat_count = 160
    model.build(ds, eval_ds, cfg.data_sink_steps, new_repeat_count)
    sync = AllreduceSync()
    import numpy as np
    sync(Tensor(np.ones(1), mstype.float32))
    print('using build>>>>>>>>>>>>>>>>>>>>>>>>>>')

    model.eval(eval_ds, dataset_sink_mode=(cfg.enable_data_sink == "true"))
    print(f'new_repeat_count: {new_repeat_count}')
    print(f'train start: {datetime.datetime.now()}')
