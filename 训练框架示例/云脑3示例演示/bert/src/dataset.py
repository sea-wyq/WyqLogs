...
class BucketDatasetGenerator:
    ...
    def __init__(self, dataset, batch_size, bucket_list=None, train_steps=None):
        self.dataset = dataset
        self.batch_size = batch_size
        self.bucket_list = bucket_list
        self.train_steps = 20000
        self.random_list_strategy3()
        self.random_list = [bucket_list[i] for i in self.random_list]
        self._init_variables()
        self.target_clip_length = bucket_list[0]
        self.clipped_length_from = bucket_list[-1]
    ...
    def random_list_strategy3(self, p128=0.5):
        p128 = os.getenv("P128", p128)
        print("p128: ", p128)
        p128 = int(float(p128) * 100)
        p512 = 100 - p128
        rpt = self.__len__() // 100
        rmd = self.__len__() % 100
        self.random_list = np.array(([0] * p128 + [1] * p512) * rpt + [0] * rmd)
    ...
    def __next__(self):
        ...
        for item in self.iterator:
            if np.sum(item[1]) < 384:
                item = self.clip_data(item)
                self.data_bucket.get(self.bucket_list[0]).append(item)
            else:
                self.data_bucket.get(self.bucket_list[-1]).append(item)
    ...
    def clip_data(item):
        item_clip = item[:3]
        item_clip.append(np.array([0], np.int64))
        masked_lm_positions = np.array([0] * 76)
        masked_lm_ids = np.array([0] * 76)
        masked_lm_weights = np.array([0.0] * 76, np.float32)
        item_clip += [masked_lm_positions, masked_lm_ids, masked_lm_weights]
        return item_clip
...
def create_bert_dataset(device_num=1, rank=0, do_shuffle="true", data_dir=None, schema_dir=None, batch_size=32,
    ...
    for file_name in files:
        condition1 = dataset_format == "tfrecord" and "tfrecord" in file_name
        condition2 = dataset_format == "mindrecord" and "mindrecord" in file_name and "mindrecord.db" not in file_name
...
@dataclass
class CreateEvalDatasetInput:
    batchsize: int = field(default=32)
    device_num: int = field(default=1)
    rank: int = field(default=0)
    data_dir: str = field(default=None)
    schema_dir: str = field(default=None)
    dataset_format: str = field(default="mindrecord")
    num_samples: int = field(default=None)


def create_eval_dataset(inputs=CreateEvalDatasetInput()):
    """create evaluation dataset"""
    batchsize = inputs.batchsize
    device_num = inputs.device_num
    rank = inputs.rank
    data_dir = inputs.data_dir
    schema_dir = inputs.schema_dir
    dataset_format = inputs.dataset_format
    ...
    if os.path.isdir(data_dir):
        ...
        for file_name in files:
            condition1 = dataset_format == "tfrecord" and "tfrecord" in file_name
            condition2 = dataset_format == "mindrecord" and "mindrecord" in file_name \
                         and "mindrecord.db" not in file_name
            if condition1 or condition2:
                data_files.append(os.path.join(data_dir, file_name))
    else:
