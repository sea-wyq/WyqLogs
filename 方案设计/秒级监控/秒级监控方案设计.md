...
label_replace(kube_pod_info{namespace="hero-user",pod="$pod"}, "instance","$1","node","(.*)") * on(instance) group_right rate(node_network_transmit_bytes_total{job="node-exporter", device!~"lo.*|cali.*|docker.*"}[$__rate_interval]) * 8 > 0
